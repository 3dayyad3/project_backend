...
module.exports = (app) => {
  ...
  app.get("/api/payment/:id", async (req, res) => {
    try {
      ...
      if (dataPayment === null) {
        res.status(404).json(new RespondFormat(false, `Payment with id ${id} found`))
      }
      res.status(200).json(new RespondFormat(true, "Payments found", [dataPayment]))
    }
    catch (error) {
  ...
  app.post("/api/payment", async (req, res) => {
    try {
      ...
      if (refTicket === null) {
        res.status(404).json(new RespondFormat(false, `Ticket with id ${req.body.ref_ticket_id} not found`))
      }
      const refStock = await Stock.findOne({ref_event: refTicket.ref_event})
      if (refStock === null) {
        res.status(404).json(new RespondFormat(false, `ticket with id ${refTicket.id} have null stock`))
      }
      ...
      res.status(200).json(new RespondFormat(true, `Data saved`, [await newPayment.save()]))
    } catch (error) {
      res.status(400).json(new RespondFormat(false, error.message))
    }
  })
    ...
    try {
      ...
      if (status === "vip") {
        if (stockData.amount.regular < ticketData.amount) {
          ...
          res.status(400).send(new RespondFormat(false, `Stock only have enough ${stockData.amount.regular} regular ticket`))
        } else {
      ...
      if (status === "regular ") {
        if (stockData.amount.regular < ticketData.amount) {
          await Payment.findOneAndUpdate({id: id}, {status_payment: "fail"})
          res.status(400).send(new RespondFormat(false, `Stock only have enough ${stockData.amount.vip} vip ticket`))
        } else {
      ...
      res.status(201).json(new RespondFormat(true, `Ticket with id ${id} is payed` [await Payment.findOne({id: id})]))
    } catch (error) {
      res.status(400).json(new RespondFormat(false, error.message))
    }
  })
  ...
  app.delete("/api/payment/:id", async (req, res) => {
    try {
      ...
      if (deleted.deletedCount === 0) {
        res.status(404).json(new RespondFormat(false, `Payment with id ${id} is not avaible`))
      }
      res.status(200).json(new RespondFormat(true, `${deleted.deletedCount} data deleted`))
    } catch (error) {
