...
module.exports = (app) => {
  app.get('/api/stock', async (req, res) => {
    ...
    res
      .status(200)
      .send(new RespondFormat(true, 'Stocks data found', stockData));
  });
    ...
    try {
      ...
      if (stockData === null) {
        res
          .status(404)
          .json(
            new RespondFormat(false, `Stock with Event id ${id} not found`),
          );
      }
      res
        .status(200)
    ...
    try {
      ...
      if (deleted.deletedCount === 0) {
        res
          .status(404)
          .json(
            new RespondFormat(true, `Stock data with id ${id} not avaible`),
          );
      }
      res
        .status(200)
