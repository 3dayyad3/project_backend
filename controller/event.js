...
const putEvent = async (req, res) => {
  try {
    ...
    const currentEvent = await Event.findOne({ id: req.body.id });
    if (currentEvent === null || currentEvent === undefined) {
      res
        ...
        .json(
          new RespondFormat(
            false,
            'No Event with id ' + new String(req.body.id),
          ),
        );
    }
    ...
    if (req.body.name !== null && req.body.name !== undefined) {
      ...
      Object.assign(updatePart, { name: currentEvent.name });
    }

    if (req.body.description !== null && req.body.description !== undefined) {
      Object.assign(updatePart, {
        description: new String(req.body.description).trim(),
      });
    } else {
      Object.assign(updatePart, { description: currentEvent.description });
    }

    if (req.body.start !== null && req.body.start !== undefined) {
      Object.assign(updatePart, { start: new Date(req.body.start) });
    } else {
      Object.assign(updatePart, { start: currentEvent.start });
    }

    if (req.body.end !== null && req.body.end !== undefined) {
      Object.assign(updatePart, { end: new Date(req.body.end) });
    } else {
      Object.assign(updatePart, { end: currentEvent.end });
    }
    ...
    if (updated === null) {
      res.status(400).RespondFormat(false, 'Fail');
    }
    res
      .status(201)
      .json(
        new RespondFormat(true, 'data updated', [
          await Event.findOne({ id: req.body.id }),
        ]),
      );
  } catch (error) {
    res.status(400).json(new RespondFormat(false, error.message));
